...
| `source.type: custom_transform` | Execute declared custom treatment with named inputs and validated output semantics. | `SUPPORTED`: notebook pre-task in Asset Bundle plus review artifacts. | `REVIEW_REQUIRED`: Glue/job binding pending. | `REVIEW_REQUIRED`: Dataflow/Dataproc/BigQuery binding pending. | `REVIEW_REQUIRED`: procedure/task binding pending. | `REVIEW_REQUIRED`: notebook/pipeline binding pending. |
...
| `source.auth` | Authentication metadata or secret reference. | `REVIEW_REQUIRED`: secrets/workspace policy. | `REVIEW_REQUIRED`: IAM/Secrets Manager. | `REVIEW_REQUIRED`: IAM/Secret Manager. | `SUPPORTED_WITH_WARNINGS` for bounded REST aliases declared through `parameters.snowflake.secrets`; otherwise `REVIEW_REQUIRED`: integrations/secrets. | `REVIEW_REQUIRED`: connections/key vault. |
...
| `mode: overwrite` | Replace target or declared overwrite scope. | `SUPPORTED`: Delta overwrite. | `SUPPORTED_WITH_WARNINGS`: atomicity by format/engine. | `SUPPORTED`: BigQuery target replacement for the documented batch surface. | `SUPPORTED`: table replacement/overwrite. | `SUPPORTED_WITH_WARNINGS`: Lakehouse overwrite validated, with Fabric runtime/capacity warnings preserved. |
| `mode: upsert` | Current-state merge by keys. | `SUPPORTED`: Delta MERGE. | `SUPPORTED`: Iceberg MERGE path for the documented Glue/Iceberg surface. | `SUPPORTED`: explicit-column BigQuery MERGE. | `SUPPORTED`: MERGE. | `SUPPORTED_WITH_WARNINGS`: Lakehouse Delta merge validated, with Fabric runtime/capacity warnings preserved. |
| `mode: hash_diff_upsert` | current-state merge with hash change detection. | `SUPPORTED`: ContractForge hash prep + Delta MERGE. | `SUPPORTED_WITH_WARNINGS`: Iceberg hash semantics and workload-specific performance/concurrency still need contract-specific validation. | `REVIEW_REQUIRED`: deterministic review artifacts and production parity evidence exist; default stable planning remains gated. | `SUPPORTED_WITH_WARNINGS`: hash function/type semantics. | `SUPPORTED_WITH_WARNINGS`: Lakehouse Delta hash-diff seed/no-op/change waves validated, with Fabric runtime/capacity warnings preserved. |
| `mode: historical` | Preserve history with current/validity markers. | `SUPPORTED`: Delta MERGE pattern; Lakeflow requires review. | `REVIEW_REQUIRED`: excluded from AWS stable-final until source completeness, concurrency and history semantics are certified. | `REVIEW_REQUIRED`: deterministic BigQuery review artifacts and production benchmark evidence exist; cross-adapter production parity remains open. | `REVIEW_REQUIRED`: excluded from Snowflake stable-final until late-arriving, delete and effective-date policies are certified. | `SUPPORTED_WITH_WARNINGS`: Fabric Lakehouse historical seed/change waves validated, with Fabric runtime/capacity warnings preserved. |
| `mode: snapshot_reconcile_soft_delete` | Mark missing snapshot keys inactive/deleted. | `SUPPORTED_WITH_WARNINGS`: Delta `NOT MATCHED BY SOURCE` semantics and complete-source declaration required. | `REVIEW_REQUIRED`: excluded from AWS stable-final until complete-source and concurrency semantics are certified. | `REVIEW_REQUIRED`: deterministic BigQuery review artifacts and production benchmark evidence exist; cross-adapter production parity remains open. | `REVIEW_REQUIRED`: excluded from Snowflake stable-final until complete-source and policy semantics are certified. | `SUPPORTED_WITH_WARNINGS`: Fabric Lakehouse complete-source snapshot seed/delete waves validated, with Fabric runtime/capacity warnings preserved. |
...
| Custom treatment | `source.type: custom_transform` | Notebook task | Glue job / Step Functions review | Dataflow / Dataproc review | Procedure / task review | Notebook / pipeline review |
