...
| `contractforge-snowflake` | Stable supported surface | SQL warehouse runtime, hosted Snowpark procedure library runner with staged ZIP imports, live task graph execution, table/SQL/bounded REST/staged-file sources, Snowflake-scoped secret aliases for authenticated REST, write modes, reference hash-diff benchmarking, quality, schema policy, governance, evidence/control tables, lineage and cost reconciliation are validated for the documented `snowflake_sql_warehouse` surface. historical/snapshot soft delete are excluded from stable-final; continuous ingestion and account-feature-dependent access policy validation remain explicit review/final-certification boundaries. |
