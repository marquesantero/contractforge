...
pip install build
python -m build
# gera: dist/lakehouse_ingestion_framework-1.0.4-py3-none-any.whl
```
...
```bash
# via Databricks CLI
databricks fs cp dist/lakehouse_ingestion_framework-1.0.4-py3-none-any.whl \
...
2. Source: **Volume**
3. File path: `/Volumes/<catalog>/<schema>/libs/lakehouse_ingestion_framework-1.0.4-py3-none-any.whl`
4. Install
...
print(lakehouse_ingestion.__version__)  # 1.0.4
...
```python
%pip install /Volumes/<catalog>/<schema>/libs/lakehouse_ingestion_framework-1.0.4-py3-none-any.whl
```

Se o cluster não permite `%pip` por restrição:

```python
%pip install --index-url https://<seu_pypi_privado> lakehouse-ingestion-framework==1.0.4
```
