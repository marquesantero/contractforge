"""SSRF validation for the bounded HTTP file connector."""
...
def validate_http_file_target(url: str, *, context: str = "HTTP file source URL", resolve: bool = True) -> None:
...
def _candidate_addresses(host: str, *, resolve: bool) -> Iterable[ipaddress._BaseAddress]:
    ...
    try:
        ...
        raise ValueError(f"HTTP file target host {host!r} could not be resolved during SSRF validation: {exc}") from exc
    seen: set[str] = set()
    for info in infos:
        sockaddr = info[4]
