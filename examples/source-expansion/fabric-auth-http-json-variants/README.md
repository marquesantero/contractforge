# Fabric Authenticated HTTP JSON Variants Source Expansion

This project is a focused `F11` source-expansion smoke for authenticated
bounded `http_json` variants beyond Basic auth in the Fabric adapter.

The sources call a bounded JSON echo endpoint and validate that generated Fabric
...
```powershell
uv run python -m contractforge_fabric.cli smoke-project examples/source-expansion/fabric-auth-http-json-variants/project.yaml --environment-key fabric --max-attempts 35 --retry-after-seconds 30
```
