...
| Tenant/auth binding | `READ_ONLY_SMOKE` | Azure CLI can mint a Fabric API Bearer token for tenant `3fb3492c-48be-4ac6-ae3a-fec6a63cf4d1`; read-only workspace listing succeeded. |
| Capacity preflight | `READ_ONLY_SMOKE` | Workspace `Manager` resolves to `061fd0c8-c248-4628-a332-6adf16623533` with capacity `2139f1ea-2676-46bf-ab4f-ab248c768056`, SKU `FTL4`, assignment `Completed`, region `Brazil South`. The default Starter Pool was Medium; a `Small` single-node custom pool (`cf_small_single_node`, `55f6847a-8a82-46d0-9b61-a3ebbdb1bda9`) was created and set as the workspace default to fit FTL4. |
...
| Notebook job preflight | `READ_ONLY_SMOKE` | `contractforge-fabric preflight --check-notebook-jobs` lists Notebook job instances and warns when active jobs can consume capacity. The live smoke Notebook currently reports eight historical runs, zero active jobs and latest completed run `a843b811-79f3-46ab-a6c0-7c55de55593e`. |
| Item preflight | `READ_WRITE_SMOKE` | Workspace item listing succeeded. Created empty Lakehouse `contractforge_lh` (`07067d3f-d5e2-48c1-b526-634620e57d4b`) and generated Notebook `cf_workspace_bronze_orders` (`9832be1e-82b8-47f9-9ce1-5a6dbe42a14e`) in `Manager`; required Lakehouse/Notebook preflight now resolves cleanly. |
| Notebook execution | `READ_WRITE_SMOKE` | A contract-generated SQL smoke Notebook (`cf_default_cf_smoke_sql`, `3bbb189e-7d02-4705-9fa1-89a755cf6eae`) was submitted with default Lakehouse binding. On June 11, 2026, the public Notebook API still failed with `TooManyRequestsForCapacity` / HTTP 430 while the workspace default was Starter Pool Medium (`202c3bba-d3c0-4151-a2ce-da6419ca875d`). After switching the workspace default Spark pool to `cf_small_single_node`, the same generated smoke notebook completed successfully (`a843b811-79f3-46ab-a6c0-7c55de55593e`). |
| USGS REST/GeoJSON E2E | `READ_WRITE_E2E` | On June 11, 2026, the same USGS medallion intent used for Databricks/AWS/Snowflake ran in Fabric from bronze to silver to two gold outputs with generated notebooks only. Successful job ids: bronze `263182ca-d6d0-4907-8ecd-ed5638049da7`, silver `34bc6277-458d-436b-a34f-65e1edd0fb35`, gold daily `b32eefad-e9de-453c-89e6-f10416409d62`, gold bands `0b05b293-ec55-4e10-951d-71b3da77a32d`; validation contract `d9b10759-30a1-4320-879f-43382e8edc2f` checked bronze count = 1 and downstream counts > 0. |
