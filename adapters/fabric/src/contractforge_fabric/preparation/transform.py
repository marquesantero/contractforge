"""Render portable Fabric notebook transform preparation steps."""
...
from contractforge_fabric.preparation.shape import can_render_shape, render_shape_preparation
...
def can_render_transform(contract: SemanticContract) -> bool:
    transform = _transform_payload(contract)
    deduplicate = _as_dict(transform.get("deduplicate"))
...
def transform_requires_window(contract: SemanticContract) -> bool:
    return bool(_transform_payload(contract).get("deduplicate"))


def render_preparation(contract: SemanticContract, *, dataframe_name: str = "df") -> str:
    if not can_render_preparation(contract):
        raise ValueError(
            "Fabric notebook preparation rendering is not implemented for one or more declared transform semantics"
        )
    lines: list[str] = []
    ...
    if not lines:
        return ""
    return "\n".join(["# Apply portable transform intent.", *lines, ""])
    ...
    *,
    ...
    for key, renderer in _TRANSFORM_SECTION_RENDERERS:
        if selected_sections is not None and key not in selected_sections:
            continue
        section = _as_dict(transform.get(key))
        if section:
...
def _composite_keys(composite_keys: dict[str, Any], *, dataframe_name: str) -> list[str]:
    normalized = {str(key): _string_list(columns) for key, columns in composite_keys.items()}
    return [
...
def _deduplicate(deduplicate: dict[str, Any], *, dataframe_name: str) -> list[str]:
    keys = _string_list(deduplicate.get("keys"))
    if not keys:
...
def _order_by_from_string(order_by: str) -> list[str]:
    ...
    for clause in (item.strip() for item in order_by.split(",")):
        ...
        if parsed is None:
            raise ValueError(
                "Fabric transform.deduplicate.order_by string clauses must be simple column references. "
...
def _transform_payload(contract: SemanticContract) -> dict[str, Any]:
