"""CLI entry point for Snowflake failure-path smoke tests."""
...
from contractforge_snowflake.smoke.runner import dry_run_failure_payload, execute_failure_smoke
...
def main(
    ...
    *,
    ...
    parser = argparse.ArgumentParser(prog="contractforge-snowflake smoke-failure-paths")
    ...
    if not args.execute:
        print(json.dumps(dry_run_failure_payload(config, execute=False, execute_cleanup=args.execute_cleanup), indent=2, sort_keys=True, default=str))
        return 0
    require_smoke_connection(
        connection=args.connection,
        connect_options=args.connect_options,
        command_name="Snowflake failure-path smoke",
    )
    ...
    if connect is None or load_connect_options is None:
        raise ValueError("Snowflake failure-path smoke connector hooks were not provided")
