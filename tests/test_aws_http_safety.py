"""SSRF guard for future AWS HTTP/REST executable sources."""

from __future__ import annotations

import pytest

from contractforge_aws.security import ALLOW_PRIVATE_FLAG, validate_http_target
...
def test_public_http_urls_pass(monkeypatch: pytest.MonkeyPatch, url: str) -> None:
...
def test_non_http_schemes_are_rejected(monkeypatch: pytest.MonkeyPatch, url: str) -> None:
    monkeypatch.delenv(ALLOW_PRIVATE_FLAG, raising=False)
...
def test_private_addresses_pass_when_opt_in_flag_is_set(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_dns_resolution_disallows_link_local(monkeypatch: pytest.MonkeyPatch) -> None:
    import contractforge_core.connectors.api.rest.safety as http_safety
